...
class TestParser(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.data = caching.read_file("example.in")
    ...
    def test_endpoints(self):
        self.assertEqual(1000, self.data.endpoints[0].l_d)
        self.assertEqual(3, len(self.data.endpoints[0].caches))
        self.assertEqual(1, self.data.endpoints[0].caches[2][0])
        self.assertEqual(200, self.data.endpoints[0].caches[1][1])
